...
from utils import bias_variable, weight_variable, load_X, load_y, one_hot
...
def LSTM_Network(input):
    ...
    W = tf.Variable(tf.random.normal([n_inputs, n_hidden]))
    B = tf.Variable(tf.random.normal([n_hidden], mean=1.0)),
    # Linear activation
...
if __name__ == '__main__':
    X = tf.compat.v1.placeholder(tf.float32, [None, 128, 6])
    label_ = tf.compat.v1.placeholder(tf.float32, [None, 118])

    X_train = load_preprocess_X('../data/train/record')
    X_test = load_preprocess_X('../data/test/record')

    train_label = load_preprocess_y('../data/train/label.txt')
    test_label = load_preprocess_y('../data/test/label.txt')

    sess = tf.compat.v1.InteractiveSession(config=config)
    # lstm
    lstm_output = LSTM_Network(X)
    # cnn
    # load model
    ...
    cross_entropy = tf.math.reduce_mean(-tf.reduce_sum(label_ * tf.log(pred_Y + 1e-10), reduction_indices=[1]),
                                   name='cnn_fix_cross_entropy')
    train_step = tf.compat.v1.train.AdamOptimizer(1e-3, name='cnn_fix_train_step').minimize(cross_entropy, name='cnn_fix_minimize')
