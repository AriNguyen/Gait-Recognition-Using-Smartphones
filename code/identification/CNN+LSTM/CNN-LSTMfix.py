...
from utils import load_preprocess_X, load_preprocess_y, weight_variable, bias_variable
...
def CNN_NetWork(X):
    CNN_input = tf.transpose(X, [0, 2, 1])
...
if __name__ == '__main__':
    ...
    label = tf.placeholder(tf.float32, [None, 118])
    ...
    cross_entropy = tf.reduce_mean(-tf.reduce_sum(label * tf.log(pred_Y + 1e-10), reduction_indices=[1]),
                                   name='lstm_fix_cross_entropy')
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy, name='lstm_fix_train_global_step')
    correct_prediction = tf.equal(tf.argmax(pred_Y, 1), tf.argmax(label, 1), name='lstm_fix_correct_prediction')
